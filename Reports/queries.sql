#0 Importing into db
LOAD DATA INFILE '~/0_Consolidated_Report.csv' INTO TABLE GENERAL_ELECTION_RESULTS
...
FROM
    GENERAL_ELECTION_RESULTS GER1
        JOIN
    GENERAL_ELECTION_RESULTS GER2 ON (GER1.PARTY = GER2.PARTY
...
FROM
    ...
    FROM
        GENERAL_ELECTION_RESULTS
    GROUP BY STATE , CONSTITUENCY) AS VOTE_DETAILS
    JOIN GENERAL_ELECTION_RESULTS ger ON (VOTE_DETAILS.CONSTITUENCY = ger.CONSTITUENCY)
...
FROM
    ...
    GROUP BY STATE , CONSTITUENCY) WINNER
        JOIN
    GENERAL_ELECTION_RESULTS GER ON (GER.CONSTITUENCY = WINNER.CONSTITUENCY
        AND GER.STATE = WINNER.STATE)
WHERE
    GER.VOTES = WINNER.WINNER_VOTES
GROUP BY GER.PARTY
...
FROM
    ...
    FROM
        GENERAL_ELECTION_RESULTS
    GROUP BY STATE , CONSTITUENCY) WINNER
    JOIN GENERAL_ELECTION_RESULTS GER ON (GER.CONSTITUENCY = WINNER.CONSTITUENCY
        AND GER.STATE = WINNER.STATE)
    WHERE
        GER.VOTES = WINNER.WINNER_VOTES
...
FROM
    ...
    FROM
        GENERAL_ELECTION_RESULTS GER1
    JOIN GENERAL_ELECTION_RESULTS GER2 ON (GER1.PARTY = GER2.PARTY
        AND GER1.CANDIDATE = GER2.CANDIDATE)
    WHERE
        GER1.CONSTITUENCY <> GER2.CONSTITUENCY
